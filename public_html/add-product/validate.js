...
function isCorrect(form_arr, prev_errors) {
    var err_set = new Errors();
    ...
    for (let i = 0; i < form_arr.length; i++) {
        current_err = err_set.initValidation(form_arr[i].name, form_arr[i].value);    
        ...
        $('#' + form_arr[i].name).val(err_set.get_val(form_arr[i].name));
        
        if ((!prev_errors[form_arr[i].name] && err_set.errors[form_arr[i].name]) || (prev_errors[form_arr[i].name] && !err_set.errors[form_arr[i].name]) || (prev_errors[form_arr[i].name] && err_set.errors[form_arr[i].name] && prev_errors[form_arr[i].name] != err_set.errors[form_arr[i].name])) {
            
            errors_changed = true;
            
        }
    }
    // Send back as a result a response with TRUE as a boolean value when the check is clean and an array caontaining three items when it is not. In the latter case distinguish two situations: when the set of errors has changed (then the state in React Component will change) and when it has not, so no need to update the state in the corresponding React Componnent.
    return responseBool || [errors_changed, err_set.getValidationResults()];
}